...
SELECT name FROM countries WHERE continent = 'Europe' AND region = 'Southern Europe' ORDER BY population ASC LIMIT 1;
...
WHERE countrycode = (SELECT code FROM countries
...
-- Write SQL query here
SELECT c.name AS city, c.countrycode AS country_code
FROM cities c
WHERE c.name LIKE 'Serra%'
AND c.name <> 'Serravalle';
